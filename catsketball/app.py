...
if (
    ...
    with st.expander("Team builder"):
        player_name_id_mapping = constants.get_player_name_id_mapping()
        # league.player_map?
        n_cols = 3
        ...
        for i, team in enumerate(league.teams):
            col_selector = (i % 3)
            all_team_selectors[team.team_name] = (
                all_columns[col_selector].multiselect(
                    f"{team.team_name}", options=player_name_id_mapping
                )
            )
        draft_rosters = {} # Map team name to list of player IDs?
        draft_summary = espn_stats.summarize_league_draft(
            league, draft_rosters, include_dtdq=include_dtdq
        )
